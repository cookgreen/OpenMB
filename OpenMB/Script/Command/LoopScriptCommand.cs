...
namespace OpenMB.Script.Command
{
	public class LoopScriptCommand : ScriptCommand
	{
		...
		public override void Execute(params object[] executeArgs)
		{
			...
			int startVal = int.Parse(getVariableValue(commandArgs[0]).ToString());
			int endVal = int.Parse(getVariableValue(commandArgs[1]).ToString());
			int step = int.Parse(getVariableValue(commandArgs[2]).ToString());
